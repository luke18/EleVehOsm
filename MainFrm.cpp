...
void CMainFrame::OnUpdateLength(CCmdUI* pCmdUI)
{
  ...
  if (bViewValid)
  {
    ...
    if (nPolyline != -1)
    {
      const COSMCtrlPolyline& polyline = pView->GetCtrl().m_Polylines.ElementAt(nPolyline);
    ...
    else
    {
      ...
      if (nPolygon != -1)
      {
        const COSMCtrlPolygon& polygon = pView->GetCtrl().m_Polygons.ElementAt(nPolygon);
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) 
{
	...
	pLeftView = (COSMCtrlAppView*)m_splitter.GetPane(0,0);
    return TRUE; 
