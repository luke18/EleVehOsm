...
//Constructors / Destructors
  CGPXTrackSegment()
  {
    m_TrackPoints.SetSize(0, 1000); //Set a reasonable grow size
  }
...
//Methods
  ...
  {
    m_TrackPoints.Copy(trackSegment.m_TrackPoints);
  ...
  void Save(MSXML2::IXMLDOMDocument2Ptr& document, MSXML2::IXMLDOMNodePtr& trackSegmentNode)
  {
    ...
    INT_PTR nTrackPoints = m_TrackPoints.GetSize();
    for (INT_PTR i=0; i<nTrackPoints; i++)
      m_TrackPoints.ElementAt(i).Save(document, trackSegmentNode);
  }
  ...
  void Load(MSXML2::IXMLDOMNodePtr& trackSegmentNode)
  {
    ...
    m_TrackPoints.SetSize(0, nTrackPoints);
    for (int i=0; i<nTrackPoints; i++)
    {
      ...
      m_TrackPoints.Add(waypoint);
    }
  }

//Member variables
  CArray<CGPXWaypoint> m_TrackPoints;
};
...
//Methods
  ...
  {
    m_TrackSegments.Copy(track.m_TrackSegments);
  ...
  void Save(MSXML2::IXMLDOMDocument2Ptr& document, MSXML2::IXMLDOMNodePtr& trackNode)
  {
    ...
    INT_PTR nTrackSegments = m_TrackSegments.GetSize();
    for (INT_PTR i=0; i<nTrackSegments; i++)
    {
      ...
      m_TrackSegments.ElementAt(i).Save(document, trackSegmentNode);
  ...
  void Load(MSXML2::IXMLDOMNodePtr& trackNode)
  {
    ...
    m_TrackSegments.SetSize(0, nTrackSegments);
    for (int i=0; i<nTrackSegments; i++)
    {
      ...
      //And put it into the array
      m_TrackSegments.Add(trackSegment);
    }
  }

//Member variables
  CArray<CGPXTrackSegment, CGPXTrackSegment&> m_TrackSegments;
};
...
//Methods
  void Save(const CString& sFileName)
  {
    ...
    INT_PTR nTracks = m_Tracks.GetSize();
    for (INT_PTR i=0; i<nTracks; i++)
    {
      ...
      m_Tracks.ElementAt(i).Save(document, trackNode);
  ...
  void Load(const CString& sFileName)
  {
    ...
    m_Waypoints.SetSize(0, nWaypoints);
...
//Member variables
  CArray<CGPXTrack, CGPXTrack&>       m_Tracks;    //The array of tracks in the GPX file
  CArray<CGPXWaypoint, CGPXWaypoint&> m_Waypoints; //The array of waypoints in the GPX file
};
