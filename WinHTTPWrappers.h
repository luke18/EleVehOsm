...
//Constructors / Destructors
  ...
  CWinHTTPHandle(CWinHTTPHandle& h) : m_h(NULL)
  {
    Attach(h.Detach());
  }

  explicit CWinHTTPHandle(HINTERNET h) : m_h(h)
  {
  }
...
//Methods
  ...
  HRESULT QueryOption(DWORD dwOption, void* pBuffer, DWORD& dwBufferLength)
  {
  ...
  HRESULT SetOption(DWORD dwOption, void* pBuffer, DWORD dwBufferLength)
  {
  ...
  WINHTTP_STATUS_CALLBACK SetStatusCallback(WINHTTP_STATUS_CALLBACK lpfnInternetCallback, DWORD dwNotificationFlags = WINHTTP_CALLBACK_FLAG_ALL_NOTIFICATIONS)
  {
    return WinHttpSetStatusCallback(m_h, lpfnInternetCallback, dwNotificationFlags, NULL);
  }

  WINHTTP_STATUS_CALLBACK SetStatusCallback(DWORD dwNotificationFlags = WINHTTP_CALLBACK_FLAG_ALL_NOTIFICATIONS)
  {
...
protected:
//Methods
  static void CALLBACK _Callback(HINTERNET hInternet, DWORD_PTR dwContext, DWORD dwInternetStatus, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
  {
...
#ifdef _DEBUG
  static void TraceCallback(HINTERNET hInternet, DWORD dwInternetStatus, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
  {
    switch (dwInternetStatus)
    {
      case WINHTTP_CALLBACK_STATUS_CLOSING_CONNECTION:
      {
        TRACE(_T("Closing the connection to the server, Handle:%p\n"), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_CONNECTED_TO_SERVER:
      {
        TRACE(_T("Successfully connected to the server:%ls, Handle:%p\n"), static_cast<LPWSTR>(lpvStatusInformation), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_CONNECTING_TO_SERVER:
      {
        TRACE(_T("Connecting to the server:%ls, Handle:%p\n"), static_cast<LPWSTR>(lpvStatusInformation), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_CONNECTION_CLOSED:
      {
        TRACE(_T("Successfully closed the connection to the server, Handle:%p\n"), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE:
      {
        TRACE(_T("Data is available to be retrieved, Handle:%p, Data Available:%u\n"), hInternet, *(static_cast<DWORD*>(lpvStatusInformation)));
        break;
      }
      case WINHTTP_CALLBACK_STATUS_HANDLE_CREATED:
      {
        TRACE(_T("Handle created, Handle:%p\n"), *(static_cast<HINTERNET*>(lpvStatusInformation)));
        break;
      }
      case WINHTTP_CALLBACK_STATUS_HANDLE_CLOSING:
      {
        TRACE(_T("Handle closing, Handle:%p\n"), *(static_cast<HINTERNET*>(lpvStatusInformation)));
        break;
      }
      case WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE:
      {
        TRACE(_T("The response header has been received, Handle:%p\n"), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_INTERMEDIATE_RESPONSE:
      {
        TRACE(_T("Received an intermediate (100 level) status code message from the server, Handle:%p, Status:%u\n"), hInternet,  *(static_cast<DWORD*>(lpvStatusInformation)));
        break;
      }
      case WINHTTP_CALLBACK_STATUS_NAME_RESOLVED:
      {
        TRACE(_T("Successfully found the IP address of the server:%ls, Handle:%p\n"), static_cast<LPWSTR>(lpvStatusInformation), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_READ_COMPLETE:
      {
        TRACE(_T("Data was successfully read from the server, Data Read:%u, Handle:%p\n"), dwStatusInformationLength, hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_RECEIVING_RESPONSE:
      {
        TRACE(_T("Waiting for the server to respond to a request, Handle:%p\n"), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_REDIRECT:
      {
        TRACE(_T("An HTTP request is about to automatically redirect the request to %ls, Handle:%p\n"), static_cast<LPWSTR>(lpvStatusInformation), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_REQUEST_ERROR:
      {
        WINHTTP_ASYNC_RESULT* pResult = static_cast<WINHTTP_ASYNC_RESULT*>(lpvStatusInformation);
        TRACE(_T("An error occurred while sending an HTTP request, Error:%u, Handle:%p\n"), pResult->dwError, hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_REQUEST_SENT:
      {
        TRACE(_T("Successfully sent the information request to the server, Data Sent:%u, Handle:%p\n"), *(static_cast<DWORD*>(lpvStatusInformation)), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_RESOLVING_NAME:
      {
        TRACE(_T("Looking up the IP address of a server name:%ls, Handle:%p\n"), static_cast<LPWSTR>(lpvStatusInformation), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_RESPONSE_RECEIVED:
      {
        TRACE(_T("Successfully received a response from the server, Data Received:%u, Handle:%p\n"), *(static_cast<DWORD*>(lpvStatusInformation)), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_SECURE_FAILURE:
      {
        ...
        {
          TRACE(_T("Certification revocation checking has been enabled, but the revocation check failed to verify whether a certificate has been revoked, Handle:%p\n"), hInternet);
        }
        if (dwStatusInformation & WINHTTP_CALLBACK_STATUS_FLAG_INVALID_CERT)
        {
          TRACE(_T("SSL certificate is invalid, Handle:%p\n"), hInternet);
        }
        if (dwStatusInformation & WINHTTP_CALLBACK_STATUS_FLAG_CERT_REVOKED)
        {
          TRACE(_T("SSL certificate was revoked, Handle:%p\n"), hInternet);
        }
        if (dwStatusInformation & WINHTTP_CALLBACK_STATUS_FLAG_INVALID_CA)
        {
          TRACE(_T("The function is unfamiliar with the Certificate Authority that generated the server's certificate, Handle:%p\n"), hInternet);
        }
        if (dwStatusInformation & WINHTTP_CALLBACK_STATUS_FLAG_CERT_CN_INVALID)
        {
          TRACE(_T("SSL certificate common name (host name field) is incorrect, Handle:%p\n"), hInternet);
        }
        if (dwStatusInformation & WINHTTP_CALLBACK_STATUS_FLAG_CERT_DATE_INVALID)
        {
          TRACE(_T("SSL certificate date that was received from the server is bad. The certificate is expired, Handle:%p\n"), hInternet);
        }
        if (dwStatusInformation & WINHTTP_CALLBACK_STATUS_FLAG_SECURITY_CHANNEL_ERROR)
        {
          TRACE(_T("The application experienced an internal error loading the SSL libraries, Handle:%p\n"), hInternet);
        }
        break;
      }
      case WINHTTP_CALLBACK_STATUS_SENDING_REQUEST:
      {
        TRACE(_T("Sending the information request to the server, Handle:%p\n"), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_SENDREQUEST_COMPLETE:
      {
        TRACE(_T("The request completed successfully, Handle:%p\n"), hInternet);
        break;
      }
      case WINHTTP_CALLBACK_STATUS_WRITE_COMPLETE:
      {
        TRACE(_T("Data was successfully written to the server, Data Written:%u, Handle:%p\n"), *(static_cast<DWORD*>(lpvStatusInformation)), hInternet);
        break;
      }
      default:
      {
        TRACE(_T("Unknown status:%08X, Handle:%p\n"), dwInternetStatus, hInternet);
        break;
      }
    }
  }
  ...
  virtual HRESULT OnCallback(HINTERNET /*hInternet*/, DWORD /*dwInternetStatus*/, LPVOID /*lpvStatusInformation*/, DWORD /*dwStatusInformationLength*/)
  {
    return S_FALSE; //S_FALSE means not handled in our callback
  }

#ifdef _DEBUG
  static void TraceCallbackComplete(HRESULT hr, HINTERNET hInternet, DWORD dwInternetStatus, LPVOID /*lpvStatusInformation*/, DWORD /*dwStatusInformationLength*/)
  {
    TRACE(_T("CWinHTTPHandle::TraceCallbackComplete, HRESULT:%08X, InternetStatus:%08X, Handle:%p\n"), hr, dwInternetStatus, hInternet);
  }
#endif

  virtual HRESULT OnCallbackComplete(HRESULT /*hr*/, HINTERNET /*hInternet*/, DWORD /*dwInternetStatus*/, LPVOID /*lpvStatusInformation*/, DWORD /*dwStatusInformationLength*/)
  {
    return E_NOTIMPL;
  }
};
...
public:
//Methods
  HRESULT Initialize(LPCWSTR pwszUserAgent, DWORD dwAccessType = WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, LPCWSTR pwszProxyName = WINHTTP_NO_PROXY_NAME, 
                     LPCWSTR pwszProxyBypass = WINHTTP_NO_PROXY_BYPASS, DWORD dwFlags = WINHTTP_FLAG_ASYNC)
  {
  ...
  HRESULT GetProxyForUrl(LPCWSTR lpcwszUrl, WINHTTP_AUTOPROXY_OPTIONS& AutoProxyOptions, DWORD& dwAccessType, CStringW& sProxy, CStringW& sProxyBypass)
  {
  ...
  HRESULT SetTimeouts(int dwResolveTimeout, int dwConnectTimeout, int dwSendTimeout, int dwReceiveTimeout)
  {
...
public:
//Methods
  HRESULT Initialize(const CWinHTTPSession& session, LPCWSTR pwszServerName, INTERNET_PORT nServerPort = INTERNET_DEFAULT_PORT)
  {
...
public:
//Methods
  HRESULT Initialize(const CWinHTTPConnection& connection, LPCWSTR pwszObjectName, LPCWSTR pwszVerb = NULL, LPCWSTR pwszVersion = NULL, LPCWSTR pwszReferrer = WINHTTP_NO_REFERER,
                     LPCWSTR* ppwszAcceptTypes = WINHTTP_DEFAULT_ACCEPT_TYPES, DWORD dwFlags = 0)
  {
  ...
  HRESULT QueryAuthSchemes(DWORD& dwSupportedSchemes, DWORD& dwFirstScheme, DWORD& dwAuthTarget)
  {
  ...
  HRESULT QueryDataAvailable(DWORD* lpdwNumberOfBytesAvailable)
  {
  ...
  HRESULT ReadData(LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, DWORD* lpdwNumberOfBytesRead)
  {
  ...
  HRESULT SendRequest(LPCWSTR pwszHeaders = WINHTTP_NO_ADDITIONAL_HEADERS, DWORD dwHeadersLength = 0, LPVOID lpOptional = WINHTTP_NO_REQUEST_DATA, DWORD dwOptionalLength = 0, DWORD dwTotalLength = 0, DWORD_PTR dwContext = 0)
  {
  ...
  HRESULT SetCredentials(DWORD AuthTargets, DWORD AuthScheme, LPCWSTR pwszUserName, LPCWSTR pwszPassword)
  {
  ...
  HRESULT WriteData(LPCVOID lpBuffer, DWORD dwNumberOfBytesToWrite, DWORD* lpdwNumberOfBytesWritten)
  {
...
//Methods
  HRESULT Initialize(const CWinHTTPConnection& connection, LPCWSTR pwszObjectName, LPCWSTR pwszVerb = NULL, LPCWSTR pwszVersion = NULL, LPCWSTR pwszReferrer = WINHTTP_NO_REFERER,
                     LPCWSTR* ppwszAcceptTypes = WINHTTP_DEFAULT_ACCEPT_TYPES, DWORD dwFlags = 0, DWORD dwBufferLength = 8096)
  {
    ...
    {
      hr = m_fileToDownloadInto.Create(m_sFileToDownloadInto, GENERIC_WRITE, FILE_SHARE_READ, OPEN_ALWAYS);
      if (FAILED(hr))
        return hr;
    ...
    if (m_sFileToUpload.GetLength())
    {
      ...
      hr = m_fileToUpload.Create(m_sFileToUpload, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING);
      if (FAILED(hr))
        return hr;
  ...
  HRESULT SendRequest(LPVOID lpOptional = WINHTTP_NO_REQUEST_DATA, DWORD dwOptionalLength = 0)
  {
  ...
  virtual HRESULT DoAuthentication(DWORD dwAuthenticationScheme, DWORD /*dwFirstScheme*/, DWORD dwAuthTarget)
  {
    //What will be the return value from this method
    HRESULT hr = S_FALSE;
  ...
  virtual DWORD ChooseAuthScheme(DWORD dwSupportedSchemes, DWORD /*dwFirstScheme*/, DWORD /*dwAuthTarget*/)
  {
...
//Methods
  ...
  virtual HRESULT OnReadCompleteCallback(HINTERNET hInternet, DWORD dwInternetStatus, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
  {
  ...
  virtual HRESULT OnWriteCallback(HINTERNET /*hInternet*/, DWORD /*dwInternetStatus*/, LPVOID /*lpvStatusInformation*/, DWORD /*dwStatusInformationLength*/)
  {
  ...
  HRESULT QueryStatusCode(DWORD& dwStatusCode)
  {
    dwStatusCode = 0;
    DWORD dwStatusCodeSize = sizeof(dwStatusCode);
    HRESULT hr = QueryHeaders(WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER, WINHTTP_HEADER_NAME_BY_INDEX,
                              &dwStatusCode, dwStatusCodeSize, WINHTTP_NO_HEADER_INDEX);
    return hr;
  }

  HRESULT QueryContentLength(LONGLONG& nContentLength)
  {
    nContentLength = -1;
    ...
    HRESULT hr = QueryHeaders(WINHTTP_QUERY_CONTENT_LENGTH, WINHTTP_HEADER_NAME_BY_INDEX,
                              wszContentLength, dwBufferSize, WINHTTP_NO_HEADER_INDEX);
    if (FAILED(hr))
      return hr;
  ...
  virtual HRESULT OnHeadersAvailableCallback(HINTERNET /*hInternet*/, DWORD /*dwInternetStatus*/, LPVOID /*lpvStatusInformation*/, DWORD /*dwStatusInformationLength*/)
  {
  ...
  virtual HRESULT OnRequestErrorCallback(HINTERNET hInternet, DWORD dwInternetStatus, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
  {
  ...
  virtual HRESULT OnReadData(const void* lpvBuffer, DWORD dwBytesRead)
  {
    //What will be the return value from this method (assume the best)
    HRESULT hr = S_OK;
  ...
  {
    if (hr == S_OK)
      TRACE(_T("CAsyncWinHttpDownloader::TraceCallbackComplete, The file was successfully downloaded\n"));
    else
      TRACE(_T("CAsyncWinHttpDownloader::TraceCallbackComplete, The file was not downloaded correctly, Error:%08X\n"), hr);
  }
#endif
...
public:
//Methods
  HRESULT SendRequestSync(LPVOID lpOptional = WINHTTP_NO_REQUEST_DATA, DWORD dwOptionalLength = 0)
  {
